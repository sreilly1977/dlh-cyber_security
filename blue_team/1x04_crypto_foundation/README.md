...
# [11. The TLS Audit](https://github.com/sreilly1977/dlh-cyber_security/blob/main/blue_team/1x04_crypto_foundation/11-tls_audit.md)
...
1. A website with an **A or A+ rating** (example: `cloudflare.com`)
2. A website with a **lower rating (B or below)**, if you can find one
...
1. Supported protocol versions (**TLS 1.2 and TLS 1.3 only**)
2. Cipher suite selection (ordered by preference, with reasoning for each choice)
3. HSTS header (with appropriate max-age)
4. Any other TLS hardening parameters (session tickets, renegotiation, etc.)
...
# [12. The Disk Encryption Lab](https://github.com/sreilly1977/dlh-cyber_security/blob/main/blue_team/1x04_crypto_foundation/12-disk_encryption.md)
...
```bash
bash dd if=/dev/zero of=encrypted_volume.img bs=1M count=500
```
...
Write a script [`12-luks_manager.sh`](https://github.com/sreilly1977/dlh-cyber_security/blob/main/blue_team/1x04_crypto_foundation/12-luks_manager.sh) that accepts a mode argument:
...
1. Which encryption level is appropriate (full-disk, volume, file-level) and why
2. What happens to backup performance (estimate the overhead based on your T1 performance measurements)
...
# [14. Hardware Security and Key Management](https://github.com/sreilly1977/dlh-cyber_security/blob/main/blue_team/1x04_crypto_foundation/14-key_management.md)
...
1. **Where each key is stored** (which system/device)
2. **Who has access to each key** (which role, from your 1x03 governance structure)
3. **How keys are rotated** (frequency and process)
4. **What happens if a key is compromised** (revocation and replacement procedure)
5. **What happens if a key is lost** (recovery procedure, key escrow if appropriate)
...
# [15. The Crypto Posture Audit](https://github.com/sreilly1977/dlh-cyber_security/blob/main/blue_team/1x04_crypto_foundation/15-crypto_posture_audit.md)
...
# [17. Certificate Lifecycle Management](https://github.com/sreilly1977/dlh-cyber_security/blob/main/blue_team/1x04_crypto_foundation/17-certificate_lifecycle.md)
...
# [18. The Data Classification Matrix](https://github.com/sreilly1977/dlh-cyber_security/blob/main/blue_team/1x04_crypto_foundation/18-data_classification.md)
...
1. **Public** (example: hospital address, visiting hours)
2. **Internal** (example: staff directory, meeting schedules)
3. **Confidential** (example: financial reports, vendor contracts)
4. **Restricted** (example: patient records, credentials, encryption keys)
...
Produce a text-based decision tree that a MedDefense employee could follow to classify a new type of data:

> "Is it patient data? → Restricted. Does it contain financial information? → Confidential. Is it internal operational data? → Internal."
...
# [19. The HIPAA Crypto Checkpoint](https://github.com/sreilly1977/dlh-cyber_security/blob/main/blue_team/1x04_crypto_foundation/19-hipaa_checkpoint.md)
...
1. **§164.312(a)(2)(iv)**: Encryption and decryption of ePHI
2. **§164.312(e)(1)**: Transmission security
3. **§164.312(e)(2)(ii)**: Encryption of ePHI in transit
4. **§164.312(d)**: Authentication
