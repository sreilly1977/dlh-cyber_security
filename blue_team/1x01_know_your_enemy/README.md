...
- **Specific enough** that a reader can follow the attack step by step
- **Realistic enough** that it could actually happen
- **Connected enough** to your posture assessment that every weakness exploited has a documented gap behind it
...
- **Actor:** Organized crime / RaaS group (BlackReef profile from T2)
- **Primary Vector:** Choose from your Vector-to-Asset Matrix

### Scenario 2 - Internal: Insider Data Exfiltration
- **Actor:** Malicious insider (one of the profiles from T3)
- **Primary Vector:** Legitimate access abused

### Scenario 3 - Third Party: Supply Chain Compromise
- **Actor:** External attacker using a vendor as a stepping stone (from T5)
...
# [15. The Gap-Threat Correlation](https://github.com/sreilly1977/dlh-cyber_security/blob/main/blue_team/1x01_know_your_enemy/15-gap_threat_correlation.md)

## Goal
Cross-reference the gaps identified in 1x00 with the threats identified in 1x01 to produce an updated, threat-informed prioritization.
...
1. **Re-prioritized Gap List:** The gaps ranked by their new threat-informed risk level. Highlight any gaps that moved up or down.

2. **The Critical Three:** Identify the 3 gaps that appear most frequently across kill chains and scenarios. These are the gaps whose closure would disrupt the greatest number of attack paths.

3. **The Surprise:** Identify at least 1 gap that you rated as Medium or Low in 1x00 but that, after threat analysis, should be upgraded. Explain what changed in your understanding.

---

# [16. The Threat Priority Assessment](https://github.com/sreilly1977/dlh-cyber_security/blob/main/blue_team/1x01_know_your_enemy/16-threat_priority_assessment.md)

## Goal
Produce the definitive ranking of threats against MedDefense with actionable recommendations.

## Context
This is the verdict. Everything you have built—the actor profiles, the vectors, the surfaces, the kill chains, the STRIDE analysis, the scenarios, the gap correlation—distills into a single question:

> What are the 5 things most likely to hurt MedDefense, and what should be done about each one?

## Instructions
Produce a **Prioritized Threat Assessment** for the **Top 5 threats** to MedDefense. Rank them from **most to least critical**.
...
Actor Type: [From T6]
Primary Vector: [Most likely entry method]
Primary Target: [MedDefense asset]
Likelihood: [Critical/High/Medium/Low - justify with sector data + MedDefense specifics]
Impact: [Critical/High/Medium/Low - justify with asset criticality from 1x00]
Overall Priority: [Composite assessment]
Key Gap: [The single gap from 1x00 whose closure would most reduce this threat]
Recommended Action: [One concrete, specific action with effort estimate: Quick Win / Short-term / Long-term]
```
...
After the Top 5, write a **Strategic Recommendation** answering:

**If MedDefense could only fund 2 defensive initiatives in the next quarter based on your threat analysis, what should they be and why?**

Provide a concise paragraph with clear justification for the two selected initiatives.
...
## Context
A threat landscape is not static. It shifts with every business decision, every technology change, every public event. The ability to anticipate how threats evolve, before they materializes, separates reactive security from proactive security.
...
MedDefense partners with a university to launch a clinical trial for an experimental cardiac treatment. The trial involves:
- 500 patients
- Proprietary research protocols
- Collaboration with 3 international research institutions
- Trial data is stored on a new dedicated server at MedDefense Central

### Scenario B: Cloud Migration
MedDefense migrates its EHR system from on-premises (ehr-srv-01 / ehr-db-01) to a cloud-hosted SaaS model provided by MedTech Solutions:
- The on-premises servers are decommissioned
- All EHR access goes through the cloud

### Scenario C: Public Breach Disclosure
A regional news outlet publishes an investigative article revealing that MedDefense was the victim of the January ransomware attack on billing-srv-01:
- The article includes quotes from former patients expressing concern about their data
- The story is picked up by national healthcare media

## For Each Scenario, Answer:

1. **New Threat Actors:** Does this change attract new types of adversaries? Which ones and why?

2. **Changed Vectors:** Which attack vectors become more or less relevant?

3. **Shifted Priorities:** How do the Top 5 threats from T16 change? Do any move up or down?

4. **New Gaps:** Does this create any new security gaps that did not exist before?

5. **Net Assessment:** Does MedDefense's overall threat exposure increase, decrease or shift? (One sentence verdict with reasoning.)

---

# [18. The Threat Landscape Report](https://github.com/sreilly1977/dlh-cyber_security/blob/main/blue_team/1x01_know_your_enemy/18-threat_landscape_report.md)
