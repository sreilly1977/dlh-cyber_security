**0-whois.sh** - A bash script that extract using whois scan in csv format

**1-a_record.sh** - A bash script that retrieve the A record of a specified domain using nslookup command

**2-mx_record.sh** - A bash script that retrieve the MX record of a specified domain using nslookup command

**3-txt_record.sh** - A bash script that retrieve the TXT record of a specified domain using nslookup command

**4-dig_all.sh** - A bash script that retrieve All record of a specified domain using dig command

**5-subfinder.sh** - A bash script that fetch subdomains of given domain using subfinder command

**dig-full-output.sh** - A better dig script as many NSs no longer give full output to dig ANY
